...
function checkReady() {
    ...
    && getState(LOAD_SHELLY_CELLAR_HEATER_CONNECTED).val
...
function updatePVProduction() {
    ...
    var pv_surplus = 0;
    if(potential_pv_production > PV_GRID_FEEDIN_MAX_WATTS) {
        pv_surplus = potential_pv_production - PV_GRID_FEEDIN_MAX_WATTS;
    } 
...
function updateCellarHeaterRunTime(shelly_on) {
    if(shelly_on) {
...
function startCellarHeater(shelly_on, available_power)
{
    if(shelly_on == false) {
        ...
        setState(SCRIPT_LOAD_CELLAR_HEATER_SWITCH_COUNT_OBJ, switchCount);
    }
}

function stopCellarHeater(shelly_on, available_power)
{
    if(shelly_on == true) {
...
function controlLoads(surplus) {
    var shelly_on = getState(LOAD_SHELLY_CELLAR_HEATER_SWITCH).val;
    var available_power = shelly_on ? surplus +  LOAD_SHELLY_CELLAR_HEATER_EXPECTED_POWER_WATTS : surplus;
    if(available_power >= LOAD_SHELLY_CELLAR_HEATER_EXPECTED_POWER_WATTS) {
        startCellarHeater(shelly_on, available_power);
    } else {
        stopCellarHeater(shelly_on, available_power);
    }
    updateCellarHeaterRunTime(shelly_on);
}
